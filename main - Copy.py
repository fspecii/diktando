...
class HotkeyManager(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.is_push_to_talk = False  # Mode flag
        self.is_pressed = False  # Track if the hotkey is currently pressed
    ...
    def backup_clipboard(self):
        ...
        try:
            ...
            self.original_clipboard = QApplication.clipboard().text()
            print("Backed up clipboard using Qt")
        except Exception as e:
            print(f"Qt clipboard backup failed: {str(e)}")
            try:
                # Fall back to pyperclip
                self.original_clipboard = pyperclip.paste()
                print("Backed up clipboard using pyperclip")
    ...
    def restore_clipboard(self):
        ...
        if self.original_clipboard is not None:
            ...
            try:
                ...
                print("Restored clipboard using Qt")
            except Exception as e:
                print(f"Qt clipboard restore failed: {str(e)}")
                try:
                    # Fall back to pyperclip
                    pyperclip.copy(self.original_clipboard)
                    print("Restored clipboard using pyperclip")
                except Exception as e2:
    ...
    def paste_transcription(self):
        ...
        if self.transcription:
            try:
                # Set the clipboard to the transcription using both methods for redundancy
                try:
                    QApplication.clipboard().setText(self.transcription)
                ...
                if platform.system() == "Windows":
                    ...
                    try:
                        # For macOS, use pynput to simulate Command+V
                    ...
                    try:
                        # For Linux, use pynput to simulate Ctrl+V
                        controller = keyboard.Controller()
...
class WhisperUI(QMainWindow):
    ...
    def transcription_finished(self, text):
        ...
        # Update the UI with the original text (with timestamps)
    ...
    def copy_to_clipboard(self):
        ...
        # Use both clipboard methods for redundancy
        try:
    ...
    def hotkey_transcription_finished(self, text):
        ...
        self.log_message(f"Transcription copied to clipboard: {text}")
    ...
    def attempt_paste_transcription(self):
        ...
        try:
            ...
            if self.auto_paste_check.isChecked():
                ...
                try:
                    QApplication.clipboard().setText(self.hotkey_manager.transcription)
                    print("Set clipboard using Qt")
                except Exception as e:
                    print(f"Qt clipboard set failed: {str(e)}")
                
                try:
                    pyperclip.copy(self.hotkey_manager.transcription)
                    print("Set clipboard using pyperclip")
                except Exception as e:
                    print(f"pyperclip set failed: {str(e)}")
                ...
                # Just set the clipboard without pasting using both methods for redundancy
                try:
                    QApplication.clipboard().setText(self.hotkey_manager.transcription)
                    print("Set clipboard using Qt")
                except Exception as e:
                ...
                try:
                    pyperclip.copy(self.hotkey_manager.transcription)
                    print("Set clipboard set failed: {str(e)}")
                except Exception as e:
                    print(f"pyperclip set failed: {str(e)}")
    ...
    def clean_transcription(self, text):
        ...
        cleaned_text = re.sub(r'\[\d{2}:\d{2}:\d{2}\.\d{3} --> \d{2}:\d{2}:\d{2}\.\d{3}\]\s*', '', text)
